...
import { MapIcon, List, SlidersHorizontal, User, Search, Heart, Footprints, Laptop, Apple } from 'lucide-react';
...
const mockCampaigns: Campaign[] = [{
  ...
  itemsList: [{
    name: 'Winter coats (all sizes)',
    needed: 50,
    collected: 38
  }, {
    name: 'Warm gloves and mittens',
    needed: 30,
    collected: 25
  }, {
    name: 'Wool blankets',
  ...
  recentDonations: [{
    donor: 'Mike P.',
    items: '25 canned goods',
    time: '1h ago'
  }, {
    donor: 'Lisa K.',
    items: '15 boxes of pasta',
    time: '6h ago'
  }, {
  ...
  itemsList: [{
    name: 'Notebooks and composition books',
    needed: 40,
    collected: 20
  }, {
    ...
    name: 'Colored pencils and crayons',
    needed: 25,
    collected: 12
  }, {
    name: 'Folders and binders',
    needed: 30,
    collected: 14
  }, {
    name: 'Backpacks',
  ...
  recentDonations: [{
    ...
    donor: 'Teachers Guild',
    items: '30 supply kits',
    time: '1d ago'
  }],
  lat: 40.71,
  lng: -74.004
  ...
  recentDonations: [{
    donor: 'Pet Lovers Group',
    items: '12 bags of food',
    time: '2d ago'
  }],
  lat: 40.708,
  lng: -74.01
...
export default function Index() {
  ...
  return <div className="min-h-screen bg-background">
      ...
      <header className="sticky top-0 z-30 bg-background/80 backdrop-blur-lg border-b border-border">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center justify-between">
            {/* Logo */}
            <Link to="/about">
              <motion.div className="flex items-center gap-2 cursor-pointer hover:opacity-80 transition-opacity" initial={{
              opacity: 0,
              x: -20
            }} animate={{
              opacity: 1,
            ...
            {/* Search */}
            <motion.div className="hidden md:flex flex-1 max-w-md mx-8" initial={{
            opacity: 0,
            y: -10
          }} animate={{
            opacity: 1,
            ...
            delay: 0.1
              ...
              <div className="relative w-full">
                ...
                <Input placeholder="Search drives..." value={searchQuery} onChange={e => setSearchQuery(e.target.value)} className="pl-11 rounded-full" />
              </div>
            </motion.div>
          ...
          {/* Mobile Search */}
          <motion.div className="md:hidden mt-4" initial={{
          opacity: 0
        }} animate={{
          opacity: 1
        }} transition={{
      ...
      <div className="relative bg-gradient-to-br from-primary/10 via-background to-primary/5 border-b border-primary/10 overflow-hidden">
        ...
        <motion.div className="absolute top-4 left-[10%] text-primary/20" animate={{
        y: [0, -10, 0],
        rotate: [0, 5, 0]
      }} transition={{
        duration: 3,
        ...
        <motion.div className="absolute bottom-4 right-[15%] text-primary/20" animate={{
        y: [0, -8, 0],
        rotate: [0, -5, 0]
      }} transition={{
        duration: 2.5,
        ...
        <motion.div className="absolute top-6 right-[25%] text-primary/15" animate={{
        scale: [1, 1.2, 1],
        opacity: [0.3, 0.6, 0.3]
      }} transition={{
        duration: 2,
        ...
        <div className="container mx-auto px-4 py-8 relative z-10">
          ...
          delay: 0.3
            ...
            <motion.p className="text-xl md:text-2xl font-semibold bg-gradient-to-r from-primary via-primary/90 to-primary bg-clip-text text-transparent inline-block" animate={{
            opacity: [0.9, 1, 0.9]
          }} transition={{
            duration: 3,
            repeat: Infinity,
            ...
            <motion.div className="mt-2 flex items-center justify-center gap-2 text-sm text-muted-foreground" initial={{
            opacity: 0
          }} animate={{
            opacity: 1
          }} transition={{
            delay: 0.5
              ...
              <span>Find exactly where your donation is needed. Giving should feel easy and good—and it does here.</span>
              <span className="inline-block">
            </span>
            </motion.div>
          </motion.div>
      ...
      <main className="container mx-auto px-4 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            {/* Map View */}
            {view === 'map' && <motion.div className="h-[500px] lg:h-[700px]" initial={{
            opacity: 0
          }} animate={{
            opacity: 1
          }}>
            ...
            {/* List View */}
            {view === 'list' && <motion.div className="grid grid-cols-1 md:grid-cols-2 gap-4" initial={{
            opacity: 0
          }} animate={{
            opacity: 1
          }}>
                {filteredCampaigns.map((campaign, index) => <motion.div key={campaign.id} initial={{
              opacity: 0,
              y: 20
            }} animate={{
              opacity: 1,
      ...
      {/* Filter Panel */}
      <AnimatePresence>
        {showFilters && <FilterPanel isOpen={showFilters} onClose={() => setShowFilters(false)} filters={filters} onFilterChange={setFilters} />}
      </AnimatePresence>
      ...
      <motion.div className="fixed bottom-6 right-6 lg:hidden z-20" initial={{
      scale: 0
    }} animate={{
      scale: 1
    }} transition={{
      ...
      damping: 15
        ...
        <Button size="icon" className="w-14 h-14 rounded-full bg-primary hover:bg-primary/90 text-primary-foreground shadow-lg" onClick={() => setView(view === 'map' ? 'list' : 'map')}>
